...
def stackImagesKeypointMatching(file_list):
    ...
    stacked_image /= (len(file_list) / 2)

    stacked_image[stacked_image > 1.0] = 1

    stacked_image = (stacked_image * (255/stacked_image.max())) #.astype(np.uint8)
...
if __name__ == '__main__':
    ...
    file_list = [os.path.join(image_folder, x)
                 for x in file_list if x.endswith(('.jpg', '.png', '.bmp', '.JPG'))]
